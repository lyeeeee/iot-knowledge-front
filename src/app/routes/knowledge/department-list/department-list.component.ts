...
@Component({
  selector: 'app-knowledge-department-list',
  templateUrl: './department-list.component.html',
})
export class KnowledgeDepartmentListComponent implements OnInit {
  ...
  deleteId = '';//删除id
  departmentName: any[] = [];//存放所有已存在的部门名称，更新及新增时检查是否已存在
  fieldList: any[] = [];//存放所有领域
  // isOkLoading = false;
      ...
      this.list = Array(data.length)
        .fill({}).map((item: any, idx: number) => {
          ...
          return {
            ...
            id: data[idx].id,
            fieldName: data[idx].parent.name,
            fieldNameId: data[idx].parent.id,
          }
        });
    })
  }
...
//新增或修改  提交到后台
  submit() {
    ...
    //不存在则新增或修改
    console.log(this.form.value.selectField);
    this.http.post('api/knowledge/directory/insertField', {
      type: this.type,
      name: this.form.value.name,
      id: this.updateId,
      parentId: this.form.value.selectField,
    }).subscribe(data => {
      // console.log(data);
...
//修改弹出框（使用新增的弹出框，把值赋上即可）
  update(item) {
    ...
    this.form = this.fb.group({
      ...
      selectField: [item.fieldNameId, [Validators.required]],
    });
